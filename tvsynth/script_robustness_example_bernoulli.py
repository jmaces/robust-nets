...
from operators import noise_bernoulli
...
save_results = os.path.join(save_path, "example_S6_bernoulli.pkl")
...
def noise_type(y, eta):
    return noise_bernoulli(y, eta, p=0.05)
...
if do_plot:
    ...
    if save_plot:
        fig.savefig(
            os.path.join(
                save_path, "fig_example_S{}_bernoulli_curve.pdf".format(sample)
            ),
            bbox_inches="tight",
        )
