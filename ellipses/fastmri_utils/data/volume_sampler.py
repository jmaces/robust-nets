...
class VolumeSampler(Sampler):
    """
        Based on pytorch DistributedSampler, the difference is that all
        instances from the same volume need to go to the same node. Dataset
        example is a list of tuples (fname, instance), where fname is
        essentially the volume name (actually a filename).
    """
    ...
    def __init__(self, dataset):
        ...
        self.all_volumes_split = np.array_split(
            self.all_volume_names, self.world_size
        )
