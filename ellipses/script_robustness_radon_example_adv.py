...
# transfer the highest UNet jit pertubation to all noise levels of TV
transfer_to_TV = False
...
if do_plot:
    ...
    im = _implot(ax, X_0)
    axins = ax.inset_axes([0.02, 0.72, 0.26, 0.26])
    _implot(axins, X_0)
    axins.set_xlim(75, 105)
    axins.set_ylim(105, 75)
    axins.set_xticks([])
    axins.set_yticks([])
    ...
    if save_plot:
        fig.savefig(
            os.path.join(
                save_path, "fig_example_S{}_radon_adv_gt.pdf".format(sample)
            ),
            bbox_inches="tight",
            pad_inches=0,
        )
        ...
        for idx_noise in range(len(noise_rel)):
            ...
            if save_plot:
                fig.savefig(
                    os.path.join(
                        save_path,
                        "fig_example_S{}_radon_adv_".format(sample)
            ...
            # Xref with Poisson noise
            X_cur = results.loc[idx].X_ref[idx_noise : idx_noise + 1, 0, ...]
            ...
            ax.text(
                ...
                258,
                "rel.~$\\ell_2$-err: {:.2f}\\%".format(
                    results.loc[idx].X_ref_err[idx_noise, 0].item() * 100
                ),
                fontsize=10,
                color="white",
                horizontalalignment="left",
            ...
            ax.text(
                ...
                233,
                "PSNR: {:.2f}".format(
                    results.loc[idx].X_ref_psnr[idx_noise, 0].item()
                ),
                fontsize=10,
                color="white",
                horizontalalignment="right",
            ...
            ax.text(
                ...
                254,
                "SSIM: {:.2f}".format(
                    results.loc[idx].X_ref_ssim[idx_noise, 0].item()
                ),
                fontsize=10,
                color="white",
                horizontalalignment="right",
            ...
            if save_plot:
                fig.savefig(
                    os.path.join(
                        save_path,
                        "fig_example_S{}_radon_ref_".format(sample)
                        + method.info["name_save"]
                        + "_{:.0e}".format(noise_rel[idx_noise].item())
                        + ".pdf",
                    ),
            ...
            fig.suptitle(
                method.info["name_disp"]
                + " for rel. noise level = {:1.3f} (ref)".format(
                    noise_rel[idx_noise].item()
                )
            )
            ...
            if method.net is not None:
                ...
                if save_plot:
                    fig.savefig(
                        os.path.join(
                            save_path,
                            "fig_example_S{}_radon_adv_inverter_".format(
                                sample
                            )
                            + method.info["name_save"]
                            + "_{:.0e}".format(noise_rel[idx_noise].item())
            ...
            if transfer_to_TV and (method.net is None or idx == "UNet jit"):

                X_rec = method.reconstr(
                    results_ref.loc["UNet jit"]
                    .Y_adv[len(noise_rel) - 2, ...]
                    .unsqueeze(0)
                    .to(device),
                    noise_rel[idx_noise].float(),
                )
                ...
                ax.text(
                    ...
                    258,
                    "rel.~$\\ell_2$-err: {:.2f}\\%".format(
                        err_measure(X_rec.cpu(), X_0[0:1, ...].cpu()).item()
                        * 100
                    ),
                    fontsize=10,
                    color="white",
                ...
                axins = ax.inset_axes([0.02, 0.72, 0.26, 0.26])
                _implot(axins, X_rec)
                axins.set_xlim(157, 197)
                axins.set_ylim(52, 12)
                axins.set_xticks([])
                axins.set_yticks([])
                axins.spines["bottom"].set_color("#a1c9f4")
                axins.spines["top"].set_color("#a1c9f4")
                ...
                if save_plot:
                    fig.savefig(
                        os.path.join(
                            save_path,
                            "fig_example_S{}_radon_adv_transfer_".format(
                                sample
                            )
                            + method.info["name_save"]
                            + "_{:.0e}".format(noise_rel[idx_noise].item())
