...
from operators import prep_fft_channel, rotate_real, unprep_fft_channel
...
class CropOrPadAndResimulate(object):
    """ Regenerates simulated targets from (input, target, mask) triples.

    Returns new (input, mask, target) triples of a given shape.

    Uses a target obtained from unmasked measurements, crops or pads it,
    recalculates the measurements of the cropped data, applies the cropped mask
    to it and resimulates a new target from the masked data. This is used to
    bring several data samples to the same shape in a way that is somewhat
    adapted to the measuring process.
...
if __name__ == "__main__":
    ...
    train_larger_meta = train_meta.append(val_meta.iloc[100:])
